...
module.exports = {
  ...
  getCustomerByAccountNo: function getCustomerByAccountNo(accountNo) {
    return session.run('Match (a:' + LABEL + ' {accountNo:{accountNoVal}})-[:belong_to]->(b:' + LABEL_CUSTOMER + ') RETURN a', { accountNoVal: accountNo }).then(service.resolve()).catch(service.reject()).finally(service.finally(session, driver));
  },
