...
module.exports = {
  ...
  * @return {CLPProperty} return of object of Promise 
  */
  readAtTimestamp: function readAtTimestamp(timestamp, kwhName) {
    return session.run('MATCH (a: ' + LABEL + ' {name:{kwhNameVal}}), (b: ' + LABEL_TIMESTAMP + ' {timeId:{timestampVal}}) MERGE (a)-[r:read_at]->(b)', { kwhNameVal: kwhName, timestampVal: timestamp }).then(service.resolve()).catch(service.reject()).finally(service.finally(session, driver));
  },

  belongsToMonth: function belongsToMonth(timestamp) {
    var month = new Date(timestamp).getMonth();
    return session.run('MATCH (a: ' + LABEL_TIMESTAMP + ' {timestamp:{timestampVal}}), (b: ' + LABEL_MONTH + ' {month:{monthVal}}) MERGE (a)-[r:belong_to]->(b)', { timestampVal: timestamp, monthVal: month }).then(service.resolve()).catch(service.reject()).finally(service.finally(session, driver));
  },

  belongsToYear: function belongsToYear(timestamp) {
    var month = new Date(timestamp).getMonth();
    var year = new Date(timestamp).getFullYear();
    return session.run('MATCH (a: ' + LABEL_MONTH + ' {month:{monthVal}}), (b: ' + LABEL_YEAR + ' {year:{yearVal}}) MERGE (a)-[r:belong_to]->(b)', { monthVal: month, yearVal: year }).then(service.resolve()).catch(service.reject()).finally(service.finally(session, driver));
  },
